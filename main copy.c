...
void strategy1(float *sum, int id_processor, int processors)
{
	if (id_processor == 0)
	{
		int i;
...
void sum_numbers(int strategy, float *sum, int size_numbers_to_sum, int id_processor, int processors)
{
	if(strategy == 1 || !is_a_power_of_2(processors))
	{
		...
		strategy1(sum, id_processor, processors);
	}
	else if(strategy == 2)
...
int main(int argc, char *argv[])
{
	...
	sum_numbers(strategy, &sum, size_numbers_to_sum, id_processor, processors);
	double t1 = MPI_Wtime();
	double time = t1 - t0;
